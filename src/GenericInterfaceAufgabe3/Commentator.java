...
public class Commentator {
    ...
    static void introFighter(LinkedList<Fighter> listFighter){
        listFighter.forEach((f)-> System.out.println(f.getName() + " tritt dem Kampf bei, die Stärke beträgt: " + f.getStrength()));
    }
    ...
    static void announceWinner(LinkedList<Fighter> listWinner, double noRound){
        ...
        System.out.println(listWinner.getLast().getName() + " hat den Kampf gewonnen. Die neue Stärke betraegt: " +listWinner.getLast().getStrength()+"\n");

        if(listWinner.size()==1 && noRound == 1.0) {
            System.out.println("\n#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+");
            System.out.println(listWinner.get(0).getName() + " hat das Turnier gewonnen. Die Stärke beträgt: " + listWinner.get(0).getStrength());
            System.out.println("#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+#+");
        }
    }
